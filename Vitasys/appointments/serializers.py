...
class AppointmentRequestSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if appointment_date and appointment_time:
            ...
            # 2. 30-minute interval check
            if appointment_time.minute % 30 != 0 or appointment_time.second != 0:
                raise serializers.ValidationError("Appointments must be booked on 30-minute intervals (e.g., 09:00, 09:30).")
            ...
            if is_booked:
                raise serializers.ValidationError("This time slot is already booked or has a pending request.")

        # 4. Refined duplicate check (same doctor, same day)
        ...
        if request and request.user.role == 'patient':
            ...
            if existing_today:
                raise serializers.ValidationError({
                    "appointment_date": "You already have a pending or confirmed appointment with this doctor on this date."
                })
