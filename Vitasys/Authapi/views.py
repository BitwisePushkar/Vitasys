    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {'success': False, 'error': 'No pending signup verification found for this email.'},
                    status=status.HTTP_400_BAD_REQUEST
                )
...
class ResendSignupOTPView(APIView):
    ...
    def post(self, request):
        try:
            ...
            try:
                user = CustomUser.objects.get(email=email, is_verified=False, otp_type='verification')
            except CustomUser.DoesNotExist:
                return Response(
                    {'success': False, 'error': 'No pending signup verification found for this email.'},
                    status=status.HTTP_400_BAD_REQUEST
