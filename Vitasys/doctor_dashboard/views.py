...
class DoctorDashboardStatsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Added queue info to stats
            from appointments.utils import get_doctor_queue_info
            ...
            stats = {
                ...
                'current_queue': queue_data.get('total_waiting', 0),
    ...
    def get(self, request):
        try:
            ...
            appointments = Appointment.objects.filter(
                ...
                appointment_date=today,
                status__in=['pending', 'confirmed']
            ).select_related('patient', 'patient__user').order_by('appointment_time')[:50]
    ...
    def get(self, request):
        try:
            ...
            appointments = Appointment.objects.filter(
                ...
                status__in=['pending', 'confirmed']
            ).select_related('patient', 'patient__user').order_by('appointment_date', 'appointment_time')[:50]
            serializer = DashboardAppointmentSerializer(appointments, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except AttributeError:
            return Response(
