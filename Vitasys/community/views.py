...
class PostDetailView(APIView):
    ...
    def get(self, request, slug):
        ...
        try:
            # Allow author to see their own posts regardless of status
...
class PostUpdateView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_description="Allows doctors to update their own posts.",
        request_body=PostCreateSerializer,
        responses={
            200: openapi.Response(description="Post updated successfully"),
    ...
    def patch(self, request, slug):
        if not hasattr(request.user, 'doctor_profile'):
            return Response(
                {"error": "Only doctors can update posts"},
                status=status.HTTP_403_FORBIDDEN
            )
