...
class CustomUser(AbstractUser):
    ...
    otp_type = models.CharField(max_length=15, null=True, blank=True,
        help_text=(
            ...
            "'reset_verified' after OTP confirmed"
        )
    )
    ...
    def __str__(self):
        return (f"{self.username if self.username else self.email} "f"({self.role if self.role else 'unassigned'})")
...
class Patient(models.Model):
    ...
    city = models.CharField(max_length=100)
    phone_number = models.CharField(max_length=15, unique=True)
    emergency_contact = models.CharField(max_length=15, blank=True, null=True,help_text="Emergency contact phone number")
    emergency_email = models.EmailField(blank=True, null=True,help_text="Emergency contact email")
    is_insurance      = models.BooleanField(default=False, help_text="Has active medical insurance")
